...
public class FrmConfiguracao extends JFrame {
	...
	//private SSCaixaCombinacao cboSistema = new SSCaixaCombinacao();
	...
	private void init() {
		...
		cabecalho.setTitulo(Configurador.getFabricante().toUpperCase() +  " - Configuração");
		...
		/*GridBagConstraints gbc_cboSistema = new GridBagConstraints();
		gbc_cboSistema.insets = new Insets(3, 3, 0, 3);
		gbc_cboSistema.anchor = GridBagConstraints.NORTHWEST;
		gbc_cboSistema.fill = GridBagConstraints.HORIZONTAL;
		gbc_cboSistema.gridx = 0;
		gbc_cboSistema.gridy = 0;
		cboSistema.setRotulo("Sistema");
		form.add(cboSistema, gbc_cboSistema);*/
		...
		cboConfiguracao.setItens(Configurador.CONFIGURACOES, "conexao");
		cboConfiguracao.setValue(Configurador.LOCAL);
	...
	private void definirconfiguracao() {
		...
		if (configuracao != null) {
			txtUrl.setText(Configurador.getUrlPadrao(configuracao));
			txtDbLogin.setText(configuracao.getUser());
			txtDbSenha.setText(configuracao.getPass());
			txtDbRepeteSenha.setText(configuracao.getPass());
		}
	}
	private void confirmar() {
		try {
			...
			configuracao.setUrl(txtUrl.getText());
			configuracao.setUser(txtDbLogin.getText());
			configuracao.setPass(txtDbSenha.getText());
			//configuracao.setStatus(configuracao.OK);
			configuracao.setProfile(configuracao.getConexao().equals(Configurador.CONF_API)?"API":"JPA");
			configurar();
	...
	private  String configuracao() {
		...
		sb.append(Configurador.VERSAO +"=" + configuracao.getVersao()+"\n");
		sb.append(Configurador.PROFILE +"=" + configuracao.getProfile()+"\n");
		sb.append(Configurador.URL +"="+ configuracao.getUrl()  +"\n");
		if(!configuracao.getConexao().equals(Configurador.CONF_API)) {
			sb.append(Configurador.DRIVER +"=" + configuracao.getDriver()+"\n");
			sb.append(Configurador.PASS +"="+ configuracao.getPass()  +"\n");
			sb.append(Configurador.USER +"="+ configuracao.getUser()  +"\n");
			sb.append(Configurador.DIALECT +"=" + configuracao.getDialect()+"\n");
			sb.append(Configurador.DDL +"=" + configuracao.getDdl()+"\n");
			sb.append(Configurador.SHOWSQL +"=" + configuracao.getShowSql()+"\n");
			sb.append(Configurador.CONEXAO +"=" + configuracao.getConexao()+"\n");
		}
		//sb.append(Configurador.STATUS +"=" + configuracao.getStatus()+"\n");
		return sb.toString();
	}
	private  File arquivoConfiguracao() throws Exception{
		File diretorio = new File(Configurador.getConfigDir());
		File arquivo = new File(diretorio, Configurador.ARQUIVO_CONFIGURACAO);
		if (!diretorio.exists()) {
			diretorio.mkdirs();
		}
		/*File backup = new File(configuracao.getBackupDir());
		if (!backup.exists()) {
			backup.mkdirs();
		}*/
